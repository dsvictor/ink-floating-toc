/* Floating TOC - Native Neumorphic & Mathematical Bounding Box */
...
function B(plugin, view, parentUl, hdg, index) {
    let activeDoc = typeof activeWindow !== "undefined" ? activeWindow.document : window.document;
    let li = activeDoc.createElement("li");
    ...
    let s = activeDoc.createElement("div");
    s.className = "text-wrap";
    li.appendChild(s);
    
    let l = activeDoc.createElement("div");
    l.className = "line-wrap";
    let lineInner = activeDoc.createElement("div");
...
function buildEmptyBar(pluginInstance, activeDoc, ul, file) {
    let emptyLi = activeDoc.createElement("li");
    emptyLi.className = "heading-list-item empty-toc-bar";
    emptyLi.setAttribute("data-level", "1");
    let anc = activeDoc.createElement("div");
    anc.className = "toc-tooltip-anchor empty-anchor";
    let tText = file ? "No headings available" : "No note open";
    anc.setAttribute("aria-label", tText);
...
function V(appInstance, pluginInstance, view) {
    ...
    if (!view) {
        let leaf = appInstance.workspace.activeLeaf || appInstance.workspace.getLeaf(false);
    ...
    pluginInstance.BAR_STYLE_CLASSES.forEach(r => activeDoc.body.classList.remove(r));
    if (pluginInstance.settings.isDefaultPin) l.classList.add("pin");
    if (pluginInstance.settings.isDefaultHide) l.classList.add("hide");
    if (pluginInstance.settings.enableHeadingNowrap) activeDoc.body.classList.add("enable-heading-nowrap");
    if (pluginInstance.settings.enableBarHeadingText) activeDoc.body.classList.add("enable-bar-heading-text");
    if (pluginInstance.settings.enableContentOffset) activeDoc.body.classList.add("enable-content-offset");
    activeDoc.body.classList.add(pluginInstance.settings.barStyle || "enable-horizontal-solid-line-style");
    
    if (pluginInstance.settings.barBackground) {
        activeDoc.body.classList.add("enable-pill-container");
    } else {
        activeDoc.body.classList.remove("enable-pill-container");
    }
    ...
    let ul = activeDoc.createElement("ul");
    ul.className = "floating-toc";
    let toolbar = activeDoc.createElement("div");
    ...
    if (pluginInstance.headingdata.length === 0) {
        buildEmptyBar(pluginInstance, activeDoc, ul, file);
    } else {
...
function N(pluginInstance, view) {
    ...
    let r = activeDoc.createDocumentFragment();
...
var X = class extends T.Plugin {
    ...
    onload() {
        return x(this, null, function* () {
            ...
            let t = (n, o = !1) => {
                let a = Date.now();
                if (!o && a - this.lastRefreshTime < this.REFRESH_COOLDOWN) {
                    // Do nothing
                } else {
                    this.lastRefreshTime = a;
                    e(n);
                }
            };
